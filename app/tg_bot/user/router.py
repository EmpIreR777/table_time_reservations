...
from app.tg_bot.booking.state import BookingState
from app.tg_bot.user.kbs import main_user_kb, user_booking_kb, cancel_book_kb
from app.tg_bot.user.schemas import SUser
...
async def show_my_bookings(call: CallbackQuery, session_without_commit: AsyncSession):
    ...
    if count_booking:
        book = True
        text = (
            f"🎉 Отлично! У вас {count_booking} забронированных столика(ов). \n\n"
            f"Чтобы просмотреть детали брони и, при необходимости, отменить бронь, воспользуйтесь кнопками ниже. 👇"
        )
    else:
        book = False
        text = (
            "🤔 Кажется, у вас пока нет активных бронирований. \n\n"
