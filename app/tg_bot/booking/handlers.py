...
from app.tg_bot.user.kbs import main_user_kb
from app.dao.bookings_dao import BookingDAO
from app.dao.time_slots_dao import TimeSlotDAO
from app.dao.tables_dao import TableDAO
...
async def process_add_count_capacity(callback: CallbackQuery, button: Button,
    ...
    dialog_manager.dialog_data['tables'] = await TableDAO.find_all(
...
async def on_table_selected(callback: CallbackQuery, widget, dialog_manager: DialogManager, item_id: str):
    ...
    selected_table = await TableDAO.find_one_or_none_by_id(table_id, session=session)
...
async def process_date_selected(
    ...
    slots = await BookingDAO.get_available_time_slots(
        table_id=selected_table.id, booking_date=selected_date)
    if slots:
        await callback.answer(f'Выбрана дата: {selected_date}')
        dialog_manager.dialog_data['slots'] = slots
        await dialog_manager.next()
    else:
        await callback.answer(f'Нет мест на {selected_date} для стола {selected_table}!')
...
async def process_slots_selected(
    ...
    session = dialog_manager.middleware_data.get('session_without_commit')
    slot_id = int(item_id)
    selected_slot = await TimeSlotDAO.find_one_or_none_by_id(slot_id, session=session)
...
async def on_confirmation(callback: CallbackQuery, widget, dialog_manager: DialogManager, **kwargs):
    ...
    check = await BookingDAO.check_available_bookings(
        table_id=selected_table.id, time_slot_id=selected_slot.id,
        booking_date=booking_date, session=session
        )
    if check:
        await callback.answer('Приступаю к сохранению')
        add_model = SNewBooking(
            user_id=user_id, table_id=selected_table.id, time_slot_id=selected_slot.id,
            date=booking_date, status='booked')
        await BookingDAO.add(add_model, session=session)
