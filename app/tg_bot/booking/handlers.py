...
async def cancel_logic(callback: CallbackQuery, button: Button, dialog_manager: DialogManager):
    ...
    await callback.message.answer(
        "Вы отменили сценарий бронирования.", reply_markup=main_user_kb(callback.from_user.id)
    )
...
async def process_slots_selected(callback: CallbackQuery, widget, dialog_manager: DialogManager, item_id: str):
    ...
    session = dialog_manager.middleware_data.get("session_without_commit")
    slot_id = int(item_id)
    selected_slot = await TimeSlotDAO(session).find_one_or_none_by_id(slot_id)
...
async def on_confirmation(
    ...
    check = await BookingDAO(session).check_available_bookings(
        table_id=selected_table.id, time_slot_id=selected_slot.id, booking_date=booking_date
    )
    if check:
